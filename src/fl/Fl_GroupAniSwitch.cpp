   ...
   max_movel( 0 )
{
    ...
    else
    {
        switch( ani_type )
        {
            ...
            case ATYPE_JUSTHOW:
                generate_blurred_img( src, bg_blurred_img );
                break;

            case ATYPE_LEFT2RIGHT:
                generate_blurred_img( dst, bg_blurred_img );
                break;
        }
...
Fl_GroupAniSwitch::~Fl_GroupAniSwitch()
{
    ...
    fl_imgtk::discard_user_rgb_image( bg_blurred_img );
}
    ...
    if ( src != NULL )
    {
        ...
        if ( imgsurf != NULL )
        {
            ...
            if ( tmpimg != NULL )
            {
                ...
                Fl_RGB_Image* smimg = fl_imgtk::rescale( tmpimg, rsw, rsh, fl_imgtk::BILINEAR );

                if ( smimg != NULL )
                {
                    dst = fl_imgtk::rescale( smimg, src->w(), src->h(), fl_imgtk::BICUBIC );

                    fl_imgtk::discard_user_rgb_image( smimg );
                }
